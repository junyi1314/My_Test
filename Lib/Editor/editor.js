...
this.toolbar = [
    ...
    //['strikethrough','subscript','superscript','separator'],		///删除线,下标字,上标字
    ['justifyleft','justifycenter','justifyright','separator'],
    ['OrderedList','UnOrderedList','Outdent','Indent','separator'],
    ['forecolor','backcolor','separator'],
    //['HorizontalRule','InsertTable','separator'],					///横线,表格
	['separator'],
	];
...
this.btnList = {
    ...
    "about":          ['about',                '版权说明',  'editor_about(\''+objname+'\')',  'ed_about.gif'],
...
function editor_updateToolbar(objname,action) {
  ...
  if (action == "enable" || action == "disable") {
    ...
    for (var idxN in tbItems) {
      ...
      if (action == "enable")  { tbObj.disabled = false; if (isBtn) { tbObj.className = 'textbtn' }}
  ...
  for (i=0; i<IDList.length; i++) {
    ...
    if (!cmdActive)  {                                  // option is OK
      if (btnObj.className != 'textbtn') { btnObj.className = 'textbtn'; }
